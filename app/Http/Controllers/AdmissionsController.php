    ...
    public function getAdmissions()
{
    try {
        ...
        $data = admissions::all(); // Example query

        return response()->json($data);

    } catch (\Throwable $e) {
        // Log full error in storage/logs/laravel.log
        Log::error('getAdmissions error: ' . $e->getMessage(), [
            'file' => $e->getFile(),
            'line' => $e->getLine(),
            'trace' => $e->getTraceAsString()
        ]);

        // Optional: show details directly in response (for debug only)
        return response()->json([
            'error' => $e->getMessage(),
            'file' => $e->getFile(),
            'line' => $e->getLine()
